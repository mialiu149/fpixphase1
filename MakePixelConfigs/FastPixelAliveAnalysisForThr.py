...
pixelAnalysisExe   = '/home/tif/TriDAS_r34/pixel/PixelAnalysisTools/test/bin/linux/x86_64_slc6/PixelAnalysis.exe'
config             = '/home/tif/TriDAS_r34/pixel/PixelAnalysisTools/test/configuration/PixelAliveAnalysis_BpO.xml'
...
def CountDeadPixels (maxDeadPixels, outfile, excludedrocs, prePixelAliveRun):
    ...
    for roc in gDirectory.GetListOfKeys(): ## ROC folder: find one TH2F for each ROC                                                                                                                    
        ...
        effectiveDeadPixel = numDeadPixels - int(deadPixelDict[rocname])
        if (effectiveDeadPixel > maxDeadPixels):
...
def ChangeVcThr(run,key,filename,iteration,excluded,deltafilename,singleStep,largeStep,safetyMargin,skipFPix,skipBPix):
    ...
    if os.path.isfile('%s_%d.txt'%(deltafilename,iteration-1) ):
        ...
        #rocsdelta = { el[0]:int(el[1]) for el in list } #python 2.7
        rocsdelta = dict( (el[0],int(el[1])) for el in list ) #python 2.4
        print rocsdelta
            ...
            for item in group:
                if key:
                    ...
                    if (name in excludedrocs or name not in rocsInDetConfig): # exclude from the procedure rocs not in detconfis or in exlcudede.txt
                        ...
                        #print 'ROC not excluded'
                        # -- if the ROC pass PixelAlive
                        if name not in failedrocs:
                            #print 'ROC passes pixelalive'
                            if iteration == 0:
                                #print 'Iteration 0'
                                newvalue = int(value) + largeStep
                                delta    = largeStep
                            else:
                                #print 'ROC good, rocsdelta: ', rocsdelta[name]
                                if rocsdelta[name] == largeStep:
                                    if (iteration * largeStep >= 60): # don't go to low with thresholds...
                                        newvalue = int(value)
                                        delta = 0
                                    else:
                                        newvalue = int(value) + largeStep
...
def MakeNewDacSettings():
    ...
    cmd = 'cdb --insertVersionAlias dac %d Default'%newsettings
    print cmd
    #os.system(cmd)
...
parser.add_option("-p","--prePixelAlive",dest="prePixelAliveRun",type="int",help="Pre\
                  PixelAlive run number")
